...
async function renderQuinielasList() {
  ...
  await Promise.all([renderMyQuinielas(), renderQuinielasAvailable()]);
}

async function renderMyQuinielas() {
  const listEl = $('#myQuinielasList');
  listEl.innerHTML = '<p class="empty-state">Cargando…</p>';
  try {
    const mine = await api('/quinielas/entries/mine');
    listEl.innerHTML = mine.length === 0
      ? '<p class="empty-state">Aún no has llenado ninguna quiniela.</p>'
      : mine.map(e => `
        <div class="quiniela-card">
          <div class="slip-top">
            <div class="slip-desc">${escapeHtml(e.quinielaName)}</div>
          </div>
          <div class="helptext" style="margin-bottom:10px;">Código: <span class="ticket">${e.code}</span> · Llenada: ${new Date(e.createdAt).toLocaleString('es-MX')}</div>
          <button class="btn btn-ghost btn-sm" onclick="downloadQuinielaPdf('${e.id}','${e.code}')">Descargar PDF</button>
        </div>
      `).join('');
  } catch (err) {
    showError(listEl, err.message);
...
async function renderQuinielasAvailable() {
    ...
    listEl.innerHTML = quinielas.map(q => `
      <div class="quiniela-card">
        <div class="slip-top">
          ...
          ${q.cost > 0 ? `<div class="amount">$${money(q.cost)}</div>` : ''}
        </div>
        <div class="helptext" style="margin-bottom:10px;">${q.matchCount} partido${q.matchCount === 1 ? '' : 's'}${q.resultsCaptured ? ' · resultados ya capturados' : ''}${q.myEntryCount > 0 ? ` · ya la llenaste ${q.myEntryCount} vez${q.myEntryCount === 1 ? '' : 'es'}` : ''}</div>
        <button class="btn btn-primary btn-sm" onclick="openQuiniela('${q.id}')">${q.myEntryCount > 0 ? 'Llenar otra vez' : 'Llenar quiniela'}</button>
      </div>
    `).join('');
  } catch (err) {
    showError(listEl, err.message);
...
window.openQuiniela = async (id) => {
  ...
  try {
    ...
    if (q.myEntries.length > 0) {
      $('#quinielaMyEntriesInDetail').style.display = 'block';
      $('#quinielaMyEntriesList').innerHTML = q.myEntries.map(e => `
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
          <span class="ticket">${e.code}</span>
          <span class="helptext" style="margin:0;">${new Date(e.createdAt).toLocaleString('es-MX')}</span>
          <button class="btn btn-ghost btn-sm" onclick="downloadQuinielaPdf('${e.id}','${e.code}')">Descargar PDF</button>
        </div>
      `).join('');
    } else {
      $('#quinielaMyEntriesInDetail').style.display = 'none';
    }
...
$('#quinielaForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    alert(`¡Quiniela enviada! Tu código es ${entry.code}. Se descargó tu comprobante en PDF. Puedes llenarla otra vez si quieres, o volver a descargar tu comprobante cuando quieras desde "Mis quinielas llenadas".`);
...
window.downloadQuinielaPdf = async (entryId, code) => {
