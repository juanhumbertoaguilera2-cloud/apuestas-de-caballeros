...
$('#createQuinielaForm').addEventListener('submit', async (e) => {
  ...
  setBusy(btn, true, 'Creando…');
  try {
    await api('/admin/quinielas', { method: 'POST', body: { name: $('#qn-name').value.trim(), cost: Number($('#qn-cost').value) || 0, matches } });
...
async function loadQuinielaEntries() {
  ...
  $('#qnEntriesTable tbody').innerHTML = entries.map(e => `
    <tr>
      <td>
        <label class="switch">
          <input type="checkbox" ${e.paid ? 'checked' : ''} onchange="toggleQuinielaEntryPaid('${e.id}', this.checked)">
          <span class="switch-track"></span>
        </label>
      </td>
  ...
  `).join('') || '<tr><td colspan="5">Nadie ha llenado esta quiniela todavía</td></tr>';
}

window.toggleQuinielaEntryPaid = async (entryId, paid) => {
  try {
    await api(`/admin/quinielas/${currentAdminQuinielaId}/entries/${entryId}/paid`, { method: 'PATCH', body: { paid } });
  } catch (err) {
    alert('No se pudo actualizar: ' + err.message);
    await loadQuinielaEntries();
  }
...
function downloadQuinielaReportPdf(filter) {
  fetch(`${API}/admin/quinielas/${currentAdminQuinielaId}/report-pdf?filter=${filter}`, { headers: { Authorization: 'Bearer ' + token } })
    .then(res => { if (!res.ok) throw new Error('No se pudo generar el PDF'); return res.blob(); })
    .then(blob => {
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `reporte-quiniela-${filter}.pdf`;
      link.click();
      URL.revokeObjectURL(link.href);
    })
    .catch(err => alert(err.message));
}
$('#reportPdfAllBtn').addEventListener('click', () => downloadQuinielaReportPdf('all'));
$('#reportPdfPaidBtn').addEventListener('click', () => downloadQuinielaReportPdf('paid'));
