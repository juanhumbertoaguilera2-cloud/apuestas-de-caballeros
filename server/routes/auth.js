...
// Recuperación de contraseña sin correo: si defines la variable de entorno
// ADMIN_RECOVERY_KEY en el servidor, cualquiera que la conozca (solo tú)
// puede restablecer la contraseña de una cuenta sin saber la anterior.
// Si no defines esa variable, esta función queda desactivada.
// ---------------------------------------------------------------------------
router.post('/recover', (req, res) => {
  ...
  if (!configuredKey) {
    return res.status(503).json({ error: 'La recuperación por llave no está configurada en este servidor todavía.' });
  }
  if (!username || !recoveryKey || !newPassword) {
    return res.status(400).json({ error: 'Faltan datos' });
  }
  if (recoveryKey !== configuredKey) {
    return res.status(401).json({ error: 'La llave de recuperación no es correcta' });
  }
